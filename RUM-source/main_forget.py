...
def main():
        ...
        if args.mem_proxy == 'confidence' or args.mem_proxy == 'max_conf' or args.mem_proxy == 'entropy' or args.mem_proxy == 'bi_acc':
            ...
            if args.sequential:
                if args.mem == 'high':
                    ...
                    mc = m_con_idx[:args.num_indexes_to_replace // 3]
                    lc = l_con_idx[-args.num_indexes_to_replace // 3:]
                    retain_dataset_indices = all_indices - set(hc + mc + lc)
                print('check 2, retain set size: ', len(retain_dataset_indices))
            else:
                retain_dataset_indices = all_indices - set(forget_dataset_indices)
                print('check 2, retain set size: ', len(retain_dataset_indices))
            ...
            if args.unlearn_step is not None:  # sequential unlearning with multiple steps
                ...
                if args.sequential or args.unlearn == 'seq_mix':
                    if args.unlearn_step == 0 or args.unlearn_step == 1:
                        proxy_file_name = f'assets/proxy_results/heldout_retrain_{args.dataset}_{args.arch}_s{seed}.npz'
                    else:
                        proxy_file_name = f'assets/proxy_results/heldout_retrain_{args.dataset}_{args.arch}_seq_mix_{args.uname}_step{args.unlearn_step}_num3000_groupid{args.group_index}_proxy{args.mem_proxy}_mix_s{args.seed}.npz'
                else:
                    proxy_file_name = f'assets/proxy_results/heldout_retrain_{args.dataset}_{args.arch}_{args.unlearn}_step{args.unlearn_step}_num{args.num_indexes_to_replace}_groupid{args.group_index}_proxy{args.mem_proxy}_{args.mem}_s{args.seed}.npz'
                print(f'check proxy file: {proxy_file_name}...')
                loaded_heldout_retrain = np.load(proxy_file_name, allow_pickle=True)
                loaded_klscores = loaded_heldout_retrain['kl_divergences']
                sorted_klscores = loaded_klscores[loaded_klscores[:, 0].argsort()]
                ...
                heldout_retrain_scores = sorted_klscores[:, 1]
                indices = list(range(len(train_loader.dataset)))
                forget_dataset_indices, h_ret_idx, l_ret_idx, m_ret_idx = select_fs(heldout_retrain_scores, indices, args)
    ...
    if args.mask is not None:
        ...
        args.mask = 'assets/checkpoints/0{}_original_{}_bs{}_lr0.1_seed{}_epochs{}.pth.tar'.format(
                    args.dataset, args.arch, args.batch_size, args.seed, args.epochs)
        print('check 1, which model to load: ', args.mask)
    ...
    if args.unlearn_step is not None:
        if args.mask is not None:
            ...
            args.mask = f'assets/checkpoints/0{args.dataset}_original_{args.arch}_bs{args.batch_size}_lr0.1_seed{args.seed}_epochs{args.epochs}.pth.tar'
            ...
            filename = (f'{args.unlearn}_{args.dataset}_{args.arch}_{args.class_to_replace}_num{args.num_indexes_to_replace}_'
                f'groupid{args.group_index}_proxy{args.mem_proxy}_{args.mem}_step1_seed{args.seed}.pth.tar')
                #NG_cifar100_vgg16_bn_lth_[-1]_num3000_groupidNone_proxymax_conf_mid_step1_seed532.pth
