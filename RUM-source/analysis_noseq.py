...
def main():
    ...
    df_final = df_final.assign(dataset=args.dataset, arch=args.arch, proxy=args.mem_proxy, epoch=args.epochs, alpha=args.alpha)

    fname = os.path.join(args.save_dir, "scrub_vanilla_tow.csv")

    #df_final = df_final.assign(dataset=args.dataset, arch=args.arch, proxy=args.mem_proxy, step=args.unlearn_step, gamma=args.gamma, beta=args.beta, mstep=args.msteps)

    #fname = os.path.join(args.save_dir, "hyper_results_tow.csv")
